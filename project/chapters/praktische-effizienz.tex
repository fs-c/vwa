...
Diese Anforderungen erfüllt der Code in \crMain, der zum Kompilieren des \enquote{benchmark}-Programms verwendet werden kann.
...
Options:
  ...
  -r, --randomize   Randomize the order in which the different combinations
    ...
    \end{lstlisting}
    \caption{Anwendungsbeschreibung des \enquote{benchmark}-Programms.}
\end{figure}

\ifndef{vwarelease}{
	\emph{Dieser Abschnitt ist zu erweitern. Kein Code, nur Referenz auf die ensprechenden Dateien --- die konkrete Implementation ist zu komplex, lohnt sich nicht zu erklären. Vielleicht sehr abstrakter Pseudocode? Eingehen auf die Wichtigkeit der task randomization zur Umgehung von branch prediction (siehe \prettyref{fig:sequential-runs}).}
}{}
...
\begin{figure}[htp]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot[black] table	{data/supplementary/sequential/run1/quick_random};
			\addlegendentry{1};
			\addplot[black, thick] table	{data/supplementary/sequential/run8/quick_random};
			\addlegendentry{8};
		\end{axis}
	\end{tikzpicture}
