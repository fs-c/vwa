...
\ifndef{vwarelease}{
	\emph{Referenzen anderer Analysen mit gleichen oder überlappenden Eingabearten sind beizufügen. (\cite{mcg2012} erwähnt im Kontext einer anderen Datenstruktur ein ähnliches Format wie dieses hier, könnte verwendet werden.)}

	\emph{Es wäre wohl angebracht den Code der Generatoren (und überhaupt das Konzept eines Generators in diesem Kontext) genauer darzustellen.}
}{}
...
Das Ziel ist es, eine Funktion in Abhängigkeit der Eingabegröße aufzustellen bzw. zu approximieren, welche die Laufzeit darstellt (vgl. \cite[37]{mcg2012} --- \emph{Modeling}). Werte dieser Funktion der praktischen Effizienz können mithile der Funktion \lstinline{benchmark::run} in \prettyref{lst:benchmark} ermittelt werden.
