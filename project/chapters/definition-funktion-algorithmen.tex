...
Wie schon in \prettyref{sec:algorithm-specification-overview} erwähnt, wird für Beschreibungen von Algorithmen in dieser Arbeit überwiegend Pseudocode zum Einsatz kommen. Genauer werden überwiegend die \emph{\enquote{pseudocode conventions}} aus \cite[20-22]{clrs2001} verwendet, folgend kurz zusammengefasst.
...
Bevor ein Element $A[j]$ betrachtet wird besteht die Annahme, dass die Elemente $A[1], \ldots, A[j - 1]$ sortiert sind. (Aus diesem Grund beginnt die \textbf{for}-Schleife in \prettyref{ln:insertions-for-begin} mit $j = 2$ und nicht etwa mit $j = 1$; das Subarray $A[1 \twodots j - 1]$ hat für $j = 2$ nur ein Element und ist somit immer \enquote{sortiert}.)
...
\end{enumerate}
Dies passiert in der \textbf{while}-Schleife (beginnend mit \prettyref{ln:ins-inner-while}). Sie iteriert vorerst über alle möglichen Indices $i$, absteigend und beginnend mit dem höchstmöglichen ($i = j - 1$, \prettyref{ln:ins-initial-i}) bis zum kleinstmöglichen $i = 1$. Für jedes $i$ wird das Element $A[i]$ \enquote{nach oben}, zu $A[i + 1]$ verschoben (\prettyref{ln:ins-value-move}) -- somit wird Platz für das einzufügende Element geschaffen. 
...
Nach Beendigung der \textbf{for}-Schleife wurde die Eingabemenge vollständig sortiert.
