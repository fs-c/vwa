...
% Custom margins
\usepackage[centering, top=3cm, bottom=4cm]{geometry}

% For \enquote.
\usepackage{csquotes}

% Remove paragraph indent, add some parskip
\usepackage{parskip}
...
% For the \prettyref command, which allows prefixes of references to be defined in a central place (here).
...
\usepackage{caption}
\captionsetup{width=.8\textwidth, singlelinecheck=false}
...
\usepackage[style=authoryear, citereset=subsection+, maxbibnames=3, doi=false,isbn=false,url=false,eprint=false, date=year]{biblatex}
